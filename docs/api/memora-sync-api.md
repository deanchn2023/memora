...
> 版本: 3.1 | 更新: 2026-06-10
> 架构师：ADPToolkit Team
> 服务端: ADPToolkit Config Server | 端口: 3450
> v3.1 新增：记事本图片同步（详见 docs/memora-note-image-sync-api.md）

---
远程访问地址：

同步 API：http://121.5.164.126:3450/memora/sync/*
管理后台：http://121.5.164.126:3010/memora-admin.html
API 文档：docs/memora-sync-api.md
## 1. 系统架构
...
| **notes.images** | 上传/下载/删除✓ | 上传/下载/删除✓ | 只下载 | 只下载 |
...
| `category` | TEXT | 分类：`text` / `image` | ✓ |
| `image_path` | TEXT | 图片服务端路径 `{userId}/{filename}` | ✓ |
| `image_hash` | TEXT | 图片 SHA256 哈希 | ✓ |
...
> 图片文件同步需要额外调用 `/notes/images/*` 系列 API，详见 `docs/memora-note-image-sync-api.md`
...
| **v3.1 新增 notes 图片字段** | 本地 `user_notes` 加 `category`/`image_path`/`image_hash`/`image_width`/`image_height` 列 |
| **v3.1 新增图片文件同步** | 创建图片笔记时先上传图片 → push 笔记元数据；pull 时下载缺失图片 |
