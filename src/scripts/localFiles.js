...
const LocalFiles = {
  ...
  async buildIndex(forceRebuild = false) {
    ...
    try {
      const result = await window.electronAPI.localFilesIndex({
        directories: Object.keys(this.DIRECTORY_MAP).filter(k => k !== 'all'),
        forceRebuild
      });
  ...
  _renderFileCard(file) {
    ...
    const dirLabel = this.DIRECTORY_MAP[file.directory]?.label || file.directory;
  ...
  _updateIndexStatus(indexResult) {
    ...
    if (indexResult) {
      ...
      const dirs = Object.entries(indexResult.directories || {})
        .map(([k, v]) => `${this.DIRECTORY_MAP[k]?.label || k}: ${v.fileCount}`)
        .join(' · ');
