...
- **@提及**：文本中出现 `@某人 + 行动要求`（如"@Dean 你找大家收集"）→ **强制 is_task=true**
  - @提及对象是用户自己（如用户@自己的账号名）也视为待办，因为用户在给自己创建提醒
  - 示例："@congkunzhu 收集一下大家的portal项目商机" → is_task=true（@自己+行动动词"收集"）
...
- 遇到 **@任何人 + 行动要求**（如"@XX 你收集"、"@XX 安排一下"）→ 强制 `is_task=true, confidence >= 0.9`
  - **@提及对象是用户自己**（如 @自己的用户名/账号名 + 行动要求）→ 同样 `is_task=true, confidence >= 0.9`
